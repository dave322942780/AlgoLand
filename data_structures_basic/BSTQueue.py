class MinBSTQueue(object):
    cur_path = []
    _reverse = False

    def _deep_left(self, node):
        cur = node
        while cur.left:
            cur = cur.left
    ...
    def __init__(self, tree):
        ...
        self._deep_left(tree)

    def get(self):
        # edge.left is either none or visited thus can be safely ignored,
        # next smallest is edge(node), then edge[.right[.left[.left...]]],
        # if left and right children are both visited, traverse up.
        ...
        if (len(self.cur_path) == 1 and self.cur_path[0][1]) or len(self.cur_path) == 0:
            ...
            if edge_node.right:
                self.cur_path.append([edge_node.right, False])
                self._deep_left(edge_node.right)
                edge[1] = True
                return edge_node
            else:
